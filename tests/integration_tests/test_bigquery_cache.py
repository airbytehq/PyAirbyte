...
@pytest.mark.slow
def test_faker_read_to_bigquery(
    source_faker_seed_a: ab.Source,
    new_bigquery_cache: ab.BigQueryCache,
) -> None:
    """Test that the append strategy works as expected."""
    result = source_faker_seed_a.read(
        new_bigquery_cache, write_strategy="replace", force_full_refresh=True
    )
...
def test_replace_strategy(
    source_faker_seed_a: ab.Source,
    new_bigquery_cache: ab.SnowflakeCache,
) -> None:
    """Test that the append strategy works as expected."""
    for _ in range(2):
        result = source_faker_seed_a.read(
...
def test_merge_strategy(
    ...
    new_bigquery_cache: ab.caches.SnowflakeCache,
) -> None:
    ...
    result = source_faker_seed_a.read(new_bigquery_cache, write_strategy="merge")
    assert len(list(result.cache.streams["users"])) == FAKER_SCALE_A
    ...
    result = source_faker_seed_b.read(new_bigquery_cache, write_strategy="merge")
