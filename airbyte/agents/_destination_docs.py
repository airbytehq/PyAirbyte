...
_DIALECT_NOTES: Mapping[str, list[str]] = {
    "snowflake": [
        ...
        (
            "Never double-quote identifiers: quoting makes them case-sensitive, so `SELECT "
            '"id"` fails with `invalid identifier` against column `ID`. Write `SELECT id, '
            "title` (unquoted) instead."
        ),
