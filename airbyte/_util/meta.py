...
def get_http_headers() -> dict[str, str]:
    ...
    Lazily imports `fastmcp` so that `import airbyte` does not load optional
    MCP dependencies.
    """
...
def get_cloud_api_analytic_source() -> str:
    ...
    *not* an indicator of the user and/or workspace. Because it is not
    ...
    by the `DO_NOT_TRACK` environment variable. An allowlisted upstream value
    from the incoming request wins regardless of mode.
    """
