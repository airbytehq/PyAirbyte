...
class CloudWorkspace:
    ...
    def _get_connector_features(
        ...
        *,
        ...
        is enabled for agent access; an unavailable probe returns `None` since it
        does not prove disabled access. Search indexing has not launched yet, so it
        is never reported as enabled.
        """
        if not self._has_context_layer_api():
            return frozenset()

        probe_warnings = warnings if warnings is not None else []
        if connector.connector_type == ConnectorType.DESTINATION:
            if connector.definition_id not in _SQL_PASSTHROUGH_DESTINATION_DEFINITION_IDS:
                return frozenset()
            if connector._context_layer_inspect(warnings=probe_warnings) is None:  # noqa: SLF001
                return None
